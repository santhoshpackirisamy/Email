...
public class database 
{
	...
	public static void message(String sender,String receiver,String message)
	{
		try
		{  
			stmt.executeUpdate("INSERT INTO MESSAGE (sendby,receivedby,message,status,checkbox) "+ "VALUES('"
								+sender+"','"+receiver+"','"+message+"',0,0);");
		}
		catch(Exception e){System.out.println(e);}  
	}
	...
	public static void deletemsg(String id)
	{
		try
		{  
			stmt.executeUpdate("UPDATE MESSAGE set status=1 where ID=" +id+ ";");  
		}
		catch(Exception e){System.out.println(e);}  
	}
	...
	{
		ResultSet rs = stmt.executeQuery("SELECT * FROM MESSAGE WHERE receivedby ='" +name+ "' AND status=0 ORDER BY id DESC;");  
